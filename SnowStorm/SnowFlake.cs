...
namespace SnowStorm
{
	...
	sealed class SnowFlake
	{
		...
		/// </summary>
		private static List<Point[]>[] FLAKE_SIZE_TO_PATTERNS = null;
		...
		static SnowFlake()
		{
			// Create the flake patterns and rotational data
			using (new TimingScope("Static snowflake data initializaiton"))
			{
		...
		/// <summary>
		/// Creates the SnowFlake patterns.
		/// </summary>
		...
		private static void InitSnowFlakePatterns()
		{
			...
			FLAKE_SIZE_TO_PATTERNS = new List<Point[]>[(int)FLAKE_SIZES.Max() + 1];
			for (int i = 0; i < FLAKE_SIZES.Count; i++)
				FLAKE_SIZE_TO_PATTERNS[FLAKE_SIZES[i]] = new List<Point[]>();

			for (int flakeSizeIndex = 0; flakeSizeIndex < FLAKE_SIZES.Count; flakeSizeIndex++)
			{
				for (int patternIndex = 0; patternIndex < MAX_PATTERNS_FOR_SIZE; patternIndex++)
				{
					List<Point[]> set = FLAKE_SIZE_TO_PATTERNS[FLAKE_SIZES[flakeSizeIndex]];
					AddFlake(FLAKE_SIZES[flakeSizeIndex], set);
				}
			}
		}

		/// <summary>
		/// Creates a hash table that precalculates the transformation of a single point by a given
		/// angle.
		/// </summary>
		...
		public void ReInit(Vector position, Vector velocity, short inertia)
		{
			...
			List<Point[]> patternHolder = FLAKE_SIZE_TO_PATTERNS[(int)size];
